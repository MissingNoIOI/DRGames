...
namespace DRGames.Poker
{
	internal class Solver
	{
         ...
         */
		public double EvaluateRankByHighestCards(List<Card> cards, int excludeCardValue = -1, int excludeCardValue2 = -1, int limitCheck = 7, double normalize = 433175)
		{
			var i = 0;
			double sum = 0;
			var fixedSize = cards.Count() - 1;

			for (var j = fixedSize; j >= 0; j--)
			{
				var cardValue = cards[j].Rank.Value;
				if (cardValue == excludeCardValue || cardValue == excludeCardValue2)
				{
					continue;
				}

				var normalizedValue = cardValue - 2; // since CardValue is an integer between [2,14]
		...
		public HandRank GetPlayerHandRank(Player user)
		{
			...
			maxCardValue = SevenCards[6].Rank.Value;
			...
			else
			{
				...
				foreach (var suit in suits)
				{
					...
					if (suitCards.Count() >= 5)
					{
						...
						for (var i = 0; i < suitCards.Count() - 1; i++)
						{
							if (suitCards[i].Rank.Value + 1 == suitCards[i + 1].Rank.Value)
							{
								counter++;
								lastValue = suitCards[i + 1].Rank.Value;
						...
						else if (counter == 4 && lastValue == 5 && suitCards[suitCards.Count() - 1].Rank.Value == 14)
						{
							rankName = "Straight Flush";
				...
				if (rankName == null)
				{
					...
					if (duplicates.Count > 0 && duplicates[0][0] == 4)
					{
						...
						foreach (var suit in suits)
						{
							rankCards.Add(new Card(Rank.GetRank(duplicates[0][1]), suit));
						}
					}
